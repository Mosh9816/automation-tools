...
日々のチェック業務で発生する**NG 行のエラー通知メール**を  
**Outlook の下書きとして自動生成する Excel VBA ツール**です。 

 Data Check Tool で判定された NG 行をもとに、  **宛先・件名・本文を自動セットしたメールを複数作成**し、    
 作業ミスを防ぎながら効率化を実現します。
...
- ポートフォリオとしても読みやすい構成
**※ Outlook がインストールされていない環境ではメール作成部分はスキップされますが、
　処理フロー自体は正常に完了します。**
